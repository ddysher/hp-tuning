...
This guide covers how you can quickly get started using katib with minikube.

## Requirements

- A kubernetes cluster with [minikube](https://github.com/kubernetes/minikube)

## Install the system and CLI

First, install katib-cli tool.
...
The cli tool will be available at `/usr/local/bin/` directory, make sure you have access to it.

Now let's deploy katib on Kubernetes; Kubernetes manifests are located at `manifests` directory.

```bash
$ ./deploy.sh
```

The script will create a `katib` namespace and create the following components:
